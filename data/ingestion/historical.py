...
class HistoricalDataDownloader:
    """
    Download historical tick and candle data from Deriv API.

    DEPRECATED: Use PartitionedDownloader for memory-safe downloads.
    This class is maintained for backward compatibility but accumulates
    data in memory, which can cause OOM for large downloads.
    """
    ...
    async def download_ticks(
        self, start_time: datetime, end_time: datetime, progress_callback: Callable | None = None
    ) -> list[dict[str, Any]]:
        """
        Download tick data for a time range with pagination.

        WARNING: Accumulates all data in memory. For large downloads,
        use PartitionedDownloader.download_ticks_partitioned() instead.
        """
        all_ticks = []
        ...
        total_seconds = current_end - start_epoch
        downloaded_seconds = 0

        logger.info(f"Downloading ticks from {start_time} to {end_time}")

        while current_end > start_epoch:
            try:
                response = await self.client.api.ticks_history(
                    {
                        "ticks_history": self.symbol,
                        "count": self.MAX_TICKS_PER_REQUEST,
                        "end": current_end,
                ...
                # Add ticks
                for price, epoch in zip(prices, times, strict=False):
                    all_ticks.append({"epoch": epoch, "quote": price})

                # Move window back
                if times:
                    current_end = min(times) - 1
                    downloaded_seconds = int(end_time.timestamp()) - current_end

                    if progress_callback:
                        progress_callback(downloaded_seconds, total_seconds)

                # Rate limiting
                await asyncio.sleep(0.5)

            except Exception as e:
                logger.error(f"Error downloading ticks: {e}")
                await asyncio.sleep(2)
                continue

        # Sort by time (oldest first)
        all_ticks.sort(key=lambda x: x["epoch"])
        logger.info(f"Downloaded {len(all_ticks)} ticks")
    ...
    async def download_candles(
        ...
        """
        Download candle data for a time range.

        WARNING: Accumulates all data in memory. For large downloads,
        use PartitionedDownloader.download_candles_partitioned() instead.
        """
        all_candles = []
        current_end = int(end_time.timestamp())
        start_epoch = int(start_time.timestamp())
        ...
        logger.info(f"Downloading candles from {start_time} to {end_time}")
        ...
        while current_end > start_epoch:
            try:
                ...
                if candles:
                    ...
                    if progress_callback:
                        progress_callback(len(all_candles), total_candles_estimate)
    ...
    def save_ticks_parquet(
        ...
        download_duration: float | None = None,
    ) -> tuple[Path, DatasetMetadata | None]:
        """Save ticks to Parquet file with integrity checks and versioning."""
        if not HAS_PANDAS:
            raise RuntimeError("pandas required for Parquet storage")
        ...
        if check_integrity:
            checker = IntegrityChecker()
            ticks, report = checker.generate_report(ticks, data_type="ticks", auto_dedupe=True)
            gaps_detected = report.gaps_detected
            duplicates_removed = report.duplicates_removed
        ...
        if compute_checksum_flag:
            metadata.sha256 = compute_checksum(filepath)
    ...
    def save_candles_parquet(
        ...
        """Save candles to Parquet file with integrity checks and versioning."""
        if not HAS_PANDAS:
            raise RuntimeError("pandas required for Parquet storage")
        ...
        if check_integrity:
            checker = IntegrityChecker()
            candles, report = checker.generate_report(
                candles, data_type="candles", granularity=granularity, auto_dedupe=True
            )
            gaps_detected = report.gaps_detected
            duplicates_removed = report.duplicates_removed
            logger.info(str(report))
    ...
    def load_ticks_parquet(self, filename: str) -> np.ndarray:
        ...
        return cast(np.ndarray, df["quote"].values)

    def load_candles_parquet(self, filename: str) -> np.ndarray:
        """Load candles from Parquet file as numpy array."""
        if not HAS_PANDAS:
            raise RuntimeError("pandas required for Parquet storage")
        ...
        # Return as OHLCV array
        return cast(np.ndarray, df[["open", "high", "low", "close", "epoch"]].values)
