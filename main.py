...
def main():
    ...
    engine = DecisionEngine(settings, shadow_store=shadow_store)
    logger.info("DecisionEngine initialized")

    # 5. Simulate Run (Mock Data)
    ...
    try:
        ...
        for k, v in probs.items():
            logger.info(f"Output {k}: {v.cpu().numpy().tolist()}")
    ...
    for i in range(batch_size):
        ...
        real_trades = engine.process_model_output(
            sample_probs,
