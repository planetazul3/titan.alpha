...
class TestCircuitBreaker(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_buy_checks_circuit_breaker(self):
        ...
        self.client.circuit_breaker._current_cooldown = 100000 # Long cooldown
        self.client.circuit_breaker._last_failure_time = asyncio.get_running_loop().time() + 1000 # Future? No, monotonic.
        # Just use time.monotonic()
        import time
        self.client.circuit_breaker._last_failure_time = time.monotonic()
    ...
    async def test_buy_records_success(self):
        ...
        self.client.circuit_breaker._failure_count = 3
        
        await self.client.buy("CALL", 10.0, 1)
        
        # Should reset failure count
        self.assertEqual(self.client.circuit_breaker._failure_count, 0)
    ...
    async def test_buy_records_failure(self):
        ...
        self.assertEqual(self.client.circuit_breaker._failure_count, 1)
        
        # Trigger threshold
        self.client.circuit_breaker.failure_threshold = 2
        with self.assertRaises(Exception):
            await self.client.buy("CALL", 10.0, 1)
    ...
    async def test_get_open_contracts_circuit_breaker(self):
        ...
        import time
        self.client.circuit_breaker._state = CircuitState.OPEN
        self.client.circuit_breaker._current_cooldown = 100000
        self.client.circuit_breaker._last_failure_time = time.monotonic()
