...
async def test_ws_disconnect():
    class MockedWs2(MockedWs):
        def __init__(self):
            ...
            self.exception = ConnectionClosedOK(Close(1000, 'test disconnect'), None)
            super().__init__()
    ...
    wsconnection.exception = ConnectionClosedOK(Close(1000, 'Closed by api'), None)
    api = deriv_api.DerivAPI(connection=wsconnection)
    await asyncio.sleep(0.1)
    ...
    wsconnection = MockedWs2()
    api = deriv_api.DerivAPI(connection=wsconnection)
    wsconnection.exception = ConnectionClosedError(Close(1234, 'Closed by remote'), None)
    last_error = api.sanity_errors.pipe(op.first(), op.to_future())
    await asyncio.sleep(0.1) # waiting for init finished
