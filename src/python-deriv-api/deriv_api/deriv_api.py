...
class DerivAPI(DerivAPICalls):
    ...
    def __init__(self, **options: str) -> None:
        ...
        self.wsconnection: Optional[ClientConnection] = None
        self.wsconnection_from_inside = True
        self.shouldReconnect = False
        self.events: Subject = Subject()
        if options.get('connection'):
            self.wsconnection: Optional[ClientConnection] = options.get('connection')
    ...
    async def api_connect(self) -> ClientConnection:
        """
            Make a websockets connection and returns ClientConnection
        Returns
        -------
            Returns websockets ClientConnection
        """
        if not self.wsconnection and self.shouldReconnect:
            self.events.on_next({'name': 'connect'})
            # Disable WS-level pings to rely on app-level heartbeat (prevents timeouts if server ignores WS Ping)
            self.wsconnection = await websockets.connect(self.api_url, ping_interval=None)
